...
class PDFReportGenerator:
    ...
    def _build_agi_score_section(self, agi_data: dict, lang: str) -> list:
        ...
        score_display = [
            [
                f"{total_score}",
                f"Level: {level.upper()}" if lang == "en" else f"等级: {level.upper()}",
            ],
        ]
    ...
    def _build_dimensions_detail(self, dimensions: dict, lang: str) -> list:
        ...
        title = (
            "Detailed Dimension Scores / 各维度详细评分"
            if lang == "zh"
            else "Detailed Dimension Scores"
        )
        elements.append(Paragraph(title, self.styles["ChineseHeading"]))

        dim_names = {
            "hours_score": (
                "Working Hours / 工作时长" if lang == "zh" else "Working Hours",
                "hours",
            ),
            "weekend_score": (
                "Weekend Policy / 周末政策" if lang == "zh" else "Weekend Policy",
                "policy",
            ),
            "overtime_score": (
                "Overtime Compensation / 加班补偿" if lang == "zh" else "Overtime Compensation",
                "compensation",
            ),
            "shift_score": ("Shift System / 轮班制度" if lang == "zh" else "Shift System", "shift"),
            "vibe_score": (
                "Work Atmosphere / 职场氛围" if lang == "zh" else "Work Atmosphere",
                "vibe",
            ),
        }

        dim_data = [
            ["Dimension / 维度", "Score / 得分", "Weight / 权重", "Weighted Score / 加权得分"]
        ]
    ...
    def _build_work_hours_section(self, stats: dict, lang: str) -> list:
        ...
        hours_data = [
            ...
            [
                "Standard Hours (40h) Compliance / 40h标准合规",
                "✓ Compliant" if avg_hours <= 40 else "⚠ Exceeded",
            ],
        ]
            ...
            TableStyle(
                [
                    ...
                    (
                        ...
                        (0, 1),
                        (-1, -1),
                        [colors.HexColor("#ecfdf5"), colors.white],
                    ),
                    ("TEXTCOLOR", (0, 1), (-1, -1), colors.HexColor("#1f2937")),
                    ("ALIGN", (1, 0), (1, -1), "CENTER"),
                    ("FONTNAME", (0, 1), (-1, -1), "Helvetica"),
