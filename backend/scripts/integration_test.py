...
class APITester:
    ...
    def print_result(self, result):
        ...
        print(f"  {icon} {result['method']:4s} {result['endpoint']:<45s} | Status: {status:>3s} | Time: {time_str:>6s}{extra}")
...
def run_all_tests():
    ...
    print(f" Started at: {__import__('datetime').datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC")
    print("="*80)
    ...
    result = {
        ...
        "has_data": len(r.json()) > 0 if r.headers.get('content-type', '').startswith('application/json') else False,
    }
    ...
    if result["success"]:
        ...
        print(f"       Pending Certifications: {data.get('certifications', {}).get('pending_review', 'N/A')}")
    ...
    # 带筛选的排行榜
    r = requests.get(f"{BASE_URL}/api/rankings?industry=tech&sort_by=agi_score&sort_order=asc&limit=5", headers=headers)
    result = {
    ...
    try:
        ...
        cursor = conn.execute(
            "SELECT id FROM certifications WHERE status='approved' LIMIT 1"
        )
        row = cursor.fetchone()
        if row:
    ...
    if cert_id_to_test and tester.token:
        r = requests.get(
            f"{BASE_URL}/api/certifications/{cert_id_to_test}/report?language=zh",
            headers=headers
        )
        result = {
            "endpoint": f"/api/certifications/{{id}}/report",
            "method": "GET",
            "status_code": r.status_code,
            "response_time_ms": "N/A",
            "success": r.status_code == 200,
