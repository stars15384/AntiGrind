...
from app.models import User, Company, WorkHourRecord
...
class TestWorkHourSubmission:
    ...
    @pytest.mark.asyncio
    async def test_submit_work_hour_record_success(self, auth_client: AsyncClient, sample_company: dict):
        workhour_data = {
    ...
    @pytest.mark.asyncio
    async def test_submit_invalid_hours_rejected(self, auth_client: AsyncClient, sample_company: dict):
        workhour_data = {
            "company_id": sample_company["id"],
            "weekly_hours": -5,
            "weekend_policy": "single_rest",
    ...
    @pytest.mark.asyncio
    async def test_submit_excessive_hours_flagged(self, auth_client: AsyncClient, sample_company: dict):
        """提交异常高工时应被标记"""
        workhour_data = {
            "company_id": sample_company["id"],
            "weekly_hours": 100,
...
class TestCompanyAGICalculation:
    ...
    @pytest.mark.asyncio
    async def test_calculate_agi_from_multiple_records(self, auth_client: AsyncClient, db_session: AsyncSession, sample_company: dict):
        """从多条工时记录计算企业AGI"""
        records_data = [
            {"weekly_hours": 42, "weekend_policy": "double_rest", "overtime_compensation": "legal", "shift_policy": "no_shift"},
            {"weekly_hours": 44, "weekend_policy": "big_small_week", "overtime_compensation": "legal", "shift_policy": "occasional"},
            {"weekly_hours": 46, "weekend_policy": "single_rest", "overtime_compensation": "fixed_subsidy", "shift_policy": "no_shift"},
        ]

        for record in records_data:
            await auth_client.post("/api/work-hours", json={
                **record,
                "company_id": sample_company["id"],
                "vibe_score": 6
            })
    ...
    @pytest.mark.asyncio
    async def test_no_records_returns_zero(self, auth_client: AsyncClient, db_session: AsyncSession, sample_company: dict):
        """没有验证记录的企业AGI为0"""
        engine = AGIEngine()
        company_agi = await engine.calculate_company_agi(sample_company["id"], db_session)
    ...
    async def test_agi_updates_company_record(self, auth_client: AsyncClient, sample_company: dict):
        ...
        for i in range(3):
            response = await auth_client.post("/api/work-hours", json={
                "company_id": sample_company["id"],
                "weekly_hours": 50 + i * 2,
