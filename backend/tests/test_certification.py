...
async def auth_client(client: AsyncClient) -> AsyncClient:
    """创建已认证的客户端"""
    user_data = {"username": "company_hr", "email": "hr@company.com", "password": "CompanyPass123!"}
...
async def sample_company(auth_client: AsyncClient, db_session: AsyncSession) -> dict:
    ...
    return {"id": str(company.id), "name": company.name, **company_data}
...
class TestCertificationApplication:
    ...
    @pytest.mark.asyncio
    async def test_apply_for_certification_success(
        self, auth_client: AsyncClient, sample_company: dict, db_session: AsyncSession
    ):
        if not sample_company or "id" not in sample_company:
            pytest.skip("无法创建测试企业")

        # 验证企业在DB中确实存在
        result = await db_session.execute(select(Company).where(Company.id == sample_company["id"]))
        ...
        assert response.status_code in [200, 201], (
            f"申请认证失败: {response.status_code} - {response.text}"
        )
        data = response.json()

        assert "company_id" in data or "id" in data, f"响应缺少ID字段: {data}"
        assert data.get("status") == "pending" or data.get("status") == "under_review", (
            f"状态异常: {data.get('status')}"
        )
    ...
    async def test_apply_to_nonexistent_company_fails(self, auth_client: AsyncClient):
        ...
        assert response.status_code in [404, 400, 422], (
            f"不存在的企业应返回错误: {response.status_code}"
        )
...
class TestCertificationReview:
    ...
    @pytest.mark.asyncio
    async def test_review_and_approve_certification(
        self, auth_client: AsyncClient, sample_company: dict
    ):
        if not sample_company:
            pytest.skip("无法创建测试企业")
        ...
        review_response = await auth_client.post(
            f"/api/certifications/{certification_id}/review", json=review_data
        )

        assert review_response.status_code == 200, (
            f"审核失败: {review_response.status_code} - {review_response.text}"
        )
    ...
    @pytest.mark.asyncio
    async def test_review_and_reject_certification(
        self, auth_client: AsyncClient, sample_company: dict
    ):
        if not sample_company:
            pytest.skip("无法创建测试企业")
        ...
        review_data = {"approved": False, "review_notes": "材料不完整，请补充加班费支付证明"}
        review_response = await auth_client.post(
            f"/api/certifications/{certification_id}/review", json=review_data
        )
    ...
    async def test_cannot_review_nonexistent_certification(self, auth_client: AsyncClient):
        review_data = {"approved": True, "review_notes": "Test review"}
        response = await auth_client.post(
            "/api/certifications/nonexistent-id/review", json=review_data
        )
        assert response.status_code in [404, 400], f"不存在的认证应返回错误: {response.status_code}"

    @pytest.mark.asyncio
    async def test_approved_certification_creates_badge(
        self, auth_client: AsyncClient, sample_company: dict
    ):
        if not sample_company:
            pytest.skip("无法创建测试企业")
        ...
        # 徽章可能不存在或需要特定权限
        assert badge_response.status_code in [200, 404], (
            f"徽章查询异常: {badge_response.status_code}"
        )
...
class TestCertificationQuery:
    ...
    async def test_get_company_certification(self, auth_client: AsyncClient, sample_company: dict):
        ...
        company_cert_response = await auth_client.get(
            f"/api/certifications/company/{sample_company['id']}"
        )
...
class TestCompanyStatusUpdate:
    ...
    @pytest.mark.asyncio
    async def test_company_status_updates_on_approval(
        self, auth_client: AsyncClient, sample_company: dict, db_session: AsyncSession
    ):
        if not sample_company:
            pytest.skip("无法创建测试企业")
    ...
    @pytest.mark.asyncio
    async def test_company_status_updates_on_rejection(
        self, auth_client: AsyncClient, sample_company: dict
    ):
        if not sample_company:
            pytest.skip("无法创建测试企业")
