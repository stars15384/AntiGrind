...
def main():
    ...
    status, auth_data = test_api_endpoint('POST', '/api/auth/login', data=login_data)  # 使用表单数据
    token = None
    ...
    if token:
        status, users_data = test_api_endpoint('GET', '/api/admin/users?limit=3', headers=auth_headers)
        if status == 200 and 'users' in users_data:
            users_list = users_data['users']
            print(f'[OK] 成功获取 {len(users_list)} 个用户 (总计: {users_data.get("pagination", {}).get("total", 0)})')
            for i, user in enumerate(users_list[:2], 1):
                print(f'  {i}. {user.get("username", "N/A")} ({user.get("email", "N/A")})')
                print(f'     角色: {user.get("role", "N/A")} | 状态: {"活跃" if user.get("is_active") else "禁用"}')
        else:
    ...
    if token:
        status, comp_data = test_api_endpoint('GET', '/api/admin/companies?limit=3', headers=auth_headers)
        if status == 200 and isinstance(comp_data, list):
            ...
            for i, comp in enumerate(comp_data[:2], 1):
                ...
                print(f'     AGI: {comp.get("agi_score", "N/A")} | 认证: {comp.get("certification_status", "N/A")}')
    ...
    if token:
        ...
        user_company_ids = set([c['id'] for c in user_companies.get('items', [])]) if 'items' in user_companies else set()

        # 从管理员端获取
        _, admin_companies = test_api_endpoint('GET', '/api/admin/companies?limit=10', headers=auth_headers)
        admin_company_ids = set([c['id'] for c in admin_companies]) if isinstance(admin_companies, list) else set()
        ...
        if user_company_ids and admin_company_ids:
            ...
            if common:
                ...
                _, admin_detail = test_api_endpoint('GET', f'/api/admin/companies/{sample_id}', headers=auth_headers)
